...
export default class Toolbar {
    ...
    initializeButtons() {
        ...
        // LungVesselROI 버튼 (폐혈관 영역 강조 기능)
        this.lungVesselROIButton = this.createButton(
            this.getLungVesselROIIcon(),
            "Lung Vessel ROI",
            "lung-vessel-roi"
        );
        ...
        this.lungVesselROIButton.onclick = () => {
            ...
            this.toggleMeasurementButton(
                this.lungVesselROIButton,
                () => {
                    // 기능 활성화: 다른 측정 도구 비활성화
                    if (this.activeMeasurementButton && this.activeMeasurementButton !== this.lungVesselROIButton) {
                        if (this.activeMeasurementButton === this.seeThroughButton) {
                            this.seeThrough.disableSeeThroughMode();
                            if (this.seeThroughSizeControls) {
                                this.seeThroughSizeControls.style.display = "none";
                            }
                        } else if (this.activeMeasurementButton === this.distanceMeasurementButton) {
                            this.measurementTool.disableMeasurementMode();
                        } else if (this.activeMeasurementButton === this.angleMeasurementButton) {
                            this.measurementTool.disableMeasurementMode();
                        }
                        this.activeMeasurementButton.classList.remove("active");
                        this.applyButtonStyle(this.activeMeasurementButton);
                    ...
                    // ROI 기능 활성화
                    this.lungVesselROI.enableLungVesselROI();
                },
                () => {
                    // 기능 비활성화
...
<svg id="a" xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 24 24" stroke="#121212" stroke-width="0.15" stroke-linecap="round" stroke-linejoin="round">
  ...
  <g>
    <path d="M21.15,21.27H2.91c-.92,0-1.67-.75-1.67-1.67v-4.41c0-.92.75-1.67,1.67-1.67h4.11c.67,0,1.27.4,1.53,1.01l.43,1.01c.1.25.35.4.61.4h4.86c.27,0,.51-.16.61-.4l.43-1.01c.26-.61.86-1.01,1.53-1.01h4.11c.92,0,1.67.75,1.67,1.67v4.41c0,.92-.75,1.67-1.67,1.67ZM2.91,14.53c-.37,0-.67.3-.67.67v4.41c0,.37.3.67.67.67h18.24c.37,0,.67-.3.67-.67v-4.41c0-.37-.3-.67-.67-.67h-4.11c-.27,0-.51.16-.61.4l-.43,1.01c-.26.61-.86,1.01-1.53,1.01h-4.86c-.67,0-1.27-.4-1.53-1.01l-.43-1.01c-.1-.25-.35-.4-.61-.4H2.91Z" fill="${color}"/>
