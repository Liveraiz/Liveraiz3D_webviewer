...
export default class StereoscopicRenderer {
    constructor(renderer, camera, scene) {
        ...
        // anamorphic stereo: 모드별 viewport 비율에 맞춰 카메라 종횡비를 유지
    ...
    syncStereoParameters() {
        ...
        this.updateStereoCameraAspect();
    }

    setAnamorphic(anamorphic) {
        this.anamorphic = Boolean(anamorphic);
        this.updateStereoCameraAspect();
        console.log(`[StereoscopicRenderer] Anamorphic set to: ${this.anamorphic}`);
    }
    ...
    getStereoCameraAspect() {
        const stereoModes = Constants.STEREOSCOPIC.MODES || {};
        const isTopBottom = this.mode === stereoModes.TOP_BOTTOM;

        if (this.anamorphic) {
            return isTopBottom ? 2 : 1;
        }

        return isTopBottom ? 1 : 0.5;
    }

    updateStereoCameraAspect() {
        this.stereoCamera.aspect = this.getStereoCameraAspect();
    }
